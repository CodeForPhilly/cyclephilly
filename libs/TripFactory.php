...
class TripFactory
{
	...
	*/
	...
	public static function getTripAttrsByFilteredUser($filterByDemographics, $filterByPurpose){
		...
		$query = "SELECT trip.id,age,gender,ethnicity,rider_type,cycling_freq.text,purpose FROM trip LEFT JOIN (user,cycling_freq) ON (user.id=trip.user_id AND user.cycling_freq=cycling_freq.id) WHERE trip.id IN (SELECT trip.id FROM trip WHERE user_id IN(SELECT user.id FROM user " . $db->escape_string($filterByDemographics) . ")  AND n_coord>'10' AND purpose IN (" . $filterByPurpose . ")) ORDER BY trip.id ASC";
