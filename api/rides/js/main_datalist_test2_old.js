...
//console.log(tripData);
		...
		// console.log("obeject size " + object_size);
		// console.log("j " + j);
			 
			// for (key in latlng){
			// 	    if (latlng.hasOwnProperty(key)) {
			// 	      object_size++;
			// 	    }
  	// 		}
    ...
    //     			if(latlng.hasOwnProperty(prop))
    //         	      object_size++;
			 // }
  			

						// console.log("This is the current length of the latlng object: " + object_size);
						// console.log("i after setting up latlngs object is  " + i);
						// console.log("j after setting up latlngs object is  " + j);
						// If i = j here then you could delete line 458, shoudl be either one off or equal 

						//console.log("This is the content of the latlngs object array:  " + latlngs);

					
								

						if (object_size > 100 && object_size < 3000) {   // only plot trips over 200 points in length 


								latlngClip = new Array();

								for (var k=Math.round(object_size*.20); k< Math.round(object_size*.80); k++){
								...
								// console.log("Clipped object is " + latlngClip);

								// latlngClip = new Array();
								// latlngClip.push(latlng2); //pushing only the middle portion of the longitude latitude array into the latlngs object for the polyline function	
								// console.log("This is the Clipped longitude latitude array at point " + i + k + ":  " + latlngClip);

								// console.log("This is the Clipped longitude latitude array:  " + latlngClip);

								loadedTrips[workingTrip].path = L.polyline(latlngClip, {color: pathColor, weight: 5, opacity: .5, smoothFactor: 5, lineCap: 'round', lineJoin: 'miter'});					
								tripsLayer.addLayer(loadedTrips[workingTrip].path);
							
								// loadedTrips[workingTrip].path = L.circle(latlngClip, 5, {color: pathColor, weight: 5, opacity: .5, smoothFactor: 5, lineCap: 'round', lineJoin: 'miter'});					
								// tripsLayer.addLayer(loadedTrips[workingTrip].path);
							


								//loadedTrips[workingTrip].path.bindPopup(workingTrip);
								// console.log("plotted new polyline at iteration " + i );
	...
	// moving resets here
								...
								// i = j; //catching the first for loop up to the end of the position of the current Working Trip in the tripData object
								// object_size=0; // reseting object size to prep for new trip
								// // latlngs = {}; // clearing contents of objects
								// // latlngClip = {};
		...
		// 			loadedTrips[workingTrip].path = L.polyline(latlngs, {color: pathColor, weight: 5, opacity: .5, smoothFactor: 5, lineCap: 'round', lineJoin: 'miter'});					
		// 			tripsLayer.addLayer(loadedTrips[workingTrip].path);
		// 			//loadedTrips[workingTrip].path.bindPopup(workingTrip);
		// 			console.log("plotted new polyline at iteration " + i );
		// 		}
		...
		// 	for (var j=Math.round(tripData.length*.50); j< Math.round(tripData.length*.6); j++){
		// 	latlng = new L.LatLng(tripData[j].latitude,tripData[j].longitude);
		// 	latlngs.push(latlng);	
...
// 			// console.log("this is the second i" + i);
...
// 				tripData[tripsToDraw[i]].path = L.polyline(tripData[tripsToDraw[i]].path._latlngs, {color: pathColor, weight: 5, opacity: .5, smoothFactor: 5, lineCap: 'round', lineJoin: 'miter'});
...
function changeColor(tripCategory){
	...
	/*
	if(showColors=="none") 
		$('#missingAttrib').css("display", "none");
	else 
